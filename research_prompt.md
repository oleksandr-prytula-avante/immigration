...
- The legacy `valid_for_selection` and `settlement_track` fields assess the broader settlement/citizenship match. Preserve that distinction: the primary PR decision now has its own `digital_nomad_pr_transition` field and does not require a citizenship path or credit for the initial nomad stay.
...
- Populate `digital_nomad_pr_transition` for each current digital-nomad or equivalent remote-worker route; use null only when no such route exists. Include the exact covered route names in `reviewed_route_names`.
...
- Keep two independent facts separate: whether a current digital-nomad/remote-worker visa exists, and whether that exact route leads to citizenship. An active dedicated route belongs in `best_routes` with `route_type="digital_nomad"` or `"remote_worker"` even when it is temporary-only and `valid_for_selection=false`. Use `settlement_track.can_lead_to_citizenship_from_this_route=false` and `classification="temporary_nomad_only"` to record that the visa exists but the citizenship path does not.
- A digital nomad route is fully valid for the citizenship-oriented selection only if it allows the applicant to live in the country with foreign contract/income and official/current sources confirm that the status can count toward, or convert into, a qualifying residence path. If it is temporary-only or explicitly excluded from settlement, keep the active route in `best_routes` but set its and the country's `valid_for_selection=false`.
